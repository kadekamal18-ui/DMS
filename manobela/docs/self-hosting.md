...
You can self-host the Suraksha Drive AI backend on your own infrastructure – a personal computer, Raspberry Pi, VPS, or any machine that can run Python and FastAPI.
...
Create `/etc/systemd/system/Suraksha Drive AI.service`:

```ini
[Unit]
Description=Suraksha Drive AI Backend
After=network.target

[Service]
Type=simple
User=<your-user>
WorkingDirectory=/path/to/Suraksha Drive AI/backend
Environment="PATH=/path/to/Suraksha Drive AI/backend/.venv/bin"
ExecStart=/path/to/Suraksha Drive AI/backend/.venv/bin/uvicorn app.main:app --host 0.0.0.0 --port 8000
Restart=always
...
```sh
sudo systemctl enable Suraksha Drive AI
sudo systemctl start Suraksha Drive AI
```

### PM2 (Cross-platform)

```sh
pm2 start run.py --name Suraksha Drive AI --interpreter python3
pm2 save
pm2 startup
```
